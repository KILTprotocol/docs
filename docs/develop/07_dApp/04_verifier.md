...
After the browser extension verified the Well-Known DID Configuration and the encrypted communication channel between the extension and the server was established, it can request the credential presentation.
...
After the user choose the credential, the extension will pass a response to the website which contains the credential presentation.
...
For that it use the Email CType.
...
After the server has build the message object, it must encrypt the message for the claimer.
