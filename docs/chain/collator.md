...
Either use `develop` or the following commit `27067d2f7a71ad0b5039c97c4319b7cbb6969efe`.
...
```bash
docker pull kiltprotocol/peregrine:27067d2f
```
...
4. Your node will only collator if it has the private key that belongs to the public key that you got in 3. Connect to your node and check with `author > hasKey(<pubKey from 3.>, aura)` if your node has the private key.
