...
export async function main(
  ...
  const signature = await didDetails.signPayload(payload, keystore, didDetails.authenticationKey.id)
  ...
  const signatureVerificationResult = 
    await DidUtils.verifyDidSignature({ message: payload, signature, expectedVerificationMethod: KeyRelationship.authentication })
